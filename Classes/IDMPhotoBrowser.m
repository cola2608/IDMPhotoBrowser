...
#import "pop/POP.h"
//#import "../../../nas/TOSMBClient/TOSMBClient.h"
...
@interface IDMPhotoBrowser () {
    ...
    UIBarButtonItem *_downloadButton, *_deleteButton, *_informationButton, *_cancelButton;
...
//        [self.navigationItem setHidesBackButton:true];
        ...
        _photosUrl = [NSMutableArray new];
        _titles = [NSMutableArray new];
        isresizedArray = [NSMutableArray new];
...
- (id)initWithPhotos:(NSArray *)photosArray withFileNames:(NSArray *)fileNamesArray{
    if ((self = [self init])) {
		_photos = [[NSMutableArray alloc] initWithArray:photosArray];
        _titles = [[NSMutableArray alloc] initWithArray:fileNamesArray];
        _photosUrl = [[NSMutableArray alloc] initWithArray:fileNamesArray];
        isresizedArray = [[NSMutableArray alloc] initWithCapacity: fileNamesArray.count];
        for (int i = 0 ; i < fileNamesArray.count ; i++)
        {
            [isresizedArray insertObject: @"" atIndex:i];
        }
	}
	return self;
}

- (id)initWithPhotos:(NSArray *)photosArray withFileNames:(NSArray *)fileNamesArray withFilePath:(NSArray *)filePathArray{
    if ((self = [self init])) {
        _photos = [[NSMutableArray alloc] initWithArray:photosArray];
        _titles = [[NSMutableArray alloc] initWithArray:fileNamesArray];
        _photosUrl = [[NSMutableArray alloc] initWithArray:filePathArray];
        isresizedArray = [[NSMutableArray alloc] initWithCapacity: fileNamesArray.count];
        for (int i = 0 ; i < fileNamesArray.count ; i++)
        {
            [isresizedArray insertObject: @"" atIndex:i];
...
- (id)initWithPhotoURLs:(NSArray *)photoURLsArray {
    if ((self = [self init])) {
		...
		_photos = [[NSMutableArray alloc] initWithArray:photosArray];
        _photosUrl = [[NSMutableArray alloc] initWithArray:photoURLsArray];
    }
	return self;
}

- (id)initWithPhotoURLs:(NSArray *)photoURLsArray withFileNames:(NSArray *)fileNamesArray {
    if ((self = [self init])) {
        NSArray *photosArray = [IDMPhoto photosWithURLs:photoURLsArray];
        _photos = [[NSMutableArray alloc] initWithArray:photosArray];
        _photosUrl = [[NSMutableArray alloc] initWithArray:photoURLsArray];
...
- (id)initWithPhotoFileURLs:(NSArray *)photoURLsArray withFileNames:(NSArray *)fileNamesArray {
    if ((self = [self init])) {
        NSArray *photosArray = [IDMPhoto photosWithFilePaths:photoURLsArray];
        _photos = [[NSMutableArray alloc] initWithArray:photosArray];
        _photosUrl = [[NSMutableArray alloc] initWithArray:photoURLsArray];
        _titles = [[NSMutableArray alloc] initWithArray:fileNamesArray];
        isresizedArray = [[NSMutableArray alloc] initWithCapacity: fileNamesArray.count];
...
- (NSURL*) getCurrentPhotoUrl {
    return _photosUrl[_currentPageIndex];
}
...
- (void)viewDidLoad {
    ...
    _toolbar.backgroundColor = [UIColor colorWithRed:0.8196 green:0.8235 blue:0.8314 alpha:1];
    ...
    // Action Button
    _actionButton = [[UIBarButtonItem alloc] initWithImage: [UIImage imageNamed:@"ic_share_gray_22pt"] style: UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
    
    // CacnelButton
//    _cancelButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemReply
//                                                                  target:self
//                                                                  action:@selector(cancelButtonPressed:)];
//    self.navigationItem.leftBarButtonItem = _cancelButton;
    
    // DownloadButton
    _downloadButton = [[UIBarButtonItem alloc] initWithImage: [UIImage imageNamed:@"ic_download_gray_22pt"] style: UIBarButtonItemStylePlain target:self action:@selector(downloadButtonPressed:)];
    
    // DeleteButton
    _deleteButton = [[UIBarButtonItem alloc] initWithImage: [UIImage imageNamed:@"ic_delete_gray_22pt"] style: UIBarButtonItemStylePlain target:self action:@selector(deleteButtonPressed:)];

    // InformationButton
    _informationButton = [[UIBarButtonItem alloc] initWithImage: [UIImage imageNamed:@"ic_information_gray_22pt"] style: UIBarButtonItemStylePlain target:self action:@selector(informationButtonPressed:)];
    // Gesture
...
//    [tracker set:kGAIScreenName value:@"VIEW_Photo"];
...
-(void) viewDidDisappear:(BOOL)animated {
     [self.SMBDelegate reloadPhotoData];
}
...
//        [items addObject:_actionButton];
    ...
    [items addObject:flexSpace];
    if (!self.isLocalFile) {
        [items addObject:_informationButton];
        [items addObject:flexSpace];
...
//            NSData *data = [[NSData alloc] initWithContentsOfFile:[_photosUrl objectAtIndex:index]];
//            UIImage *sourceImage = [UIImage imageWithData:data];
//            [_photos replaceObjectAtIndex:index withObject: [[IDMPhoto alloc] initWithImage: sourceImage]];
//        });
        
        NSData *data = [[NSData alloc] initWithContentsOfFile:[_photosUrl objectAtIndex:index]];
        if ([data length]  < 256*1024 && ![[isresizedArray objectAtIndex:index]  isEqual: @"resized"]) {
            dispatch_semaphore_t sema = dispatch_semaphore_create(0);
            dispatch_queue_t aQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
            dispatch_async(aQueue , ^{
                UIImage *image = [self imageWithImage:[UIImage imageWithData:data] scaledToSize:CGSizeMake(1024, 768)];
                [_photos replaceObjectAtIndex:index withObject: [[IDMPhoto alloc] initWithImage:image]];
                dispatch_semaphore_signal(sema);
            });
            dispatch_semaphore_wait(sema, DISPATCH_TIME_FOREVER);
        }
    }
    [isresizedArray replaceObjectAtIndex:index withObject: @"resized"];
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    // Google Cast
    [self.SMBDelegate showPhotoRemotely: _photosUrl[index]];
}
...
- (void)updateToolbar {
    // Title
    self.title = _titles[_currentPageIndex];
...
- (void)jumpToPageAtIndex:(NSUInteger)index {
	...
	if (index < [self numberOfPhotos]) {
        if (index == 0) {
            _photos[index + 1] = @"";
        }else if (index == [self numberOfPhotos] - 1){
            _photos[index - 1] = @"";
        }else {
            _photos[index + 1] = @"";
            _photos[index - 1] = @"";
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        if(!_actionButtonTitles)
        {
            ...
            NSMutableArray *ignoreArray = [[NSMutableArray alloc] init];
            [ignoreArray addObject:UIActivityTypeCopyToPasteboard];
            self.activityViewController.excludedActivityTypes = ignoreArray;
        ...
        else
        {
            ...
            self.actionsSheet.cancelButtonIndex = [self.actionsSheet addButtonWithTitle:NSLocalizedString(@"cancel", "")];
...
- (void)deleteButtonPressed:(id)sender {
    UIAlertController *alertCheckDelete = [UIAlertController
                                           alertControllerWithTitle:NSLocalizedString(@"warning", @"")
                                           message:NSLocalizedString(@"dlg_msg_delete", @"")
    ...
    UIAlertAction *okAction = [UIAlertAction
                               ...
                               {
                                   if (self.isLocalFile) {
                                       if ([self.SMBDelegate deleteLocalFileWithName:_titles[_currentPageIndex]]) {
                                           if ([self numberOfPhotos] == 1) {
                                               [self.navigationController popViewControllerAnimated:true];
                                           }else if (_currentPageIndex < [self numberOfPhotos]-1)
                                           {
                                               [_photos removeObjectAtIndex:_currentPageIndex];
                                               [_photosUrl removeObjectAtIndex:_currentPageIndex];
                                               [_titles removeObjectAtIndex:_currentPageIndex];
                                           }else {
                                               [_photos removeObjectAtIndex:_currentPageIndex];
                                               [_photosUrl removeObjectAtIndex:_currentPageIndex];
                                               [_titles removeObjectAtIndex:_currentPageIndex];
                                       ...
                                       if ([self.SMBDelegate deleteFileWithFilePath:_photosUrl[_currentPageIndex]]) {
                                           if ([self numberOfPhotos] == 1) {
                                               [self.navigationController popViewControllerAnimated:true];
                                           }else if (_currentPageIndex < [self numberOfPhotos]-1)
                                           {
...
//                                           [self jumpToPageAtIndex:_currentPageIndex];
    ...
    [alertCheckDelete addAction:cancelAction];
    [alertCheckDelete addAction:okAction];
    [self presentViewController:alertCheckDelete animated:NO completion:nil];
}

- (void)downloadButtonPressed:(id)sender {
    if ([self.SMBDelegate downloadFileWithFilePath:_photosUrl[_currentPageIndex]]) {
...
- (void)informationButtonPressed:(id)sender {
    [self.SMBDelegate showInformation:_photosUrl[_currentPageIndex]];
}
